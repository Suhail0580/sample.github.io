 ...
 * @since             1.0
 * @package           Doctreat Mobile APP API
 *
 * @wordpress-plugin
 * Plugin Name:       Doctreat Mobile APP API
 * Plugin URI:        https://codecanyon.net/user/amentotech/portfolio
 * Description:       This plugin is used for creating custom API for Doctreat WordPress Theme. Please don't update this plugin until you update your APP.
 * Version:           2.2
 * Author:            Amento Tech
 * Author URI:        https://themeforest.net/user/amentotech
 * License:           GPL-2.0+
 * License URI:       http://www.gnu.org/licenses/gpl-2.0.txt
 * Text Domain:       doctreat_api
...
if( !function_exists( 'activate_android_app' ) ) {
	function activate_android_app() {
		...
		DoctreatApp_Activator::activate();
	} 
}
/**
...
if( !function_exists( 'deactivate_android_app' ) ) {
	function deactivate_android_app() {
		...
		DoctreatApp_Deactivator::deactivate();
	}
}
...
//include APP API
$dir = DoctreatAppGlobalSettings::get_plugin_path();
 ...
 * @since    1.0.0
 */
if( !function_exists( 'run_DoctreatApp' ) ) {
	function run_DoctreatApp() {
	
		$plugin = new DoctreatApp_Core();
		$plugin->run();
...
function android_app_load_textdomain() {
  load_plugin_textdomain( 'doctreat_api', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' ); 
}
