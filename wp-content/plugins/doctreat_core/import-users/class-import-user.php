<?php 
require DoctreatGlobalSettings::get_plugin_path() . 'libraries/PhpSpreadsheet/vendor/autoload.php';
...
if ( !class_exists('Doctreat_Import_User') ) {    
    class Doctreat_Import_User {        
		 ...
		 * @import users
		 */
		public function doctreat_import_user(){
			...
			$weeklyDays	= doctreat_get_week_array();
			...
			if ( isset( $_FILES['users_csv']['tmp_name'] ) ) {
				...
				$file 			= DoctreatGlobalSettings::get_plugin_path().'/import-users/users.xlsx';
				$filetype		= 'xlsx';
				$import_type	= 'dummy';
			}
				...
				if( $userdata['role'] === 'doctors' ){
					...
					if( function_exists('doctreat_get_package_type') ){
						...
						if( !empty( $trail_doctors_id ) ){
							doctreat_update_package_data( $trail_doctors_id ,$user_id,'',1 );
						}
					}
				}
					...
					if ( $usermeta ) {
						if( $import_type === 'upload' ){
							...
							if( !is_wp_error( $post_id ) ) {
								...
								do_action('doctreat_import_extra_fields',$user_id,$usermeta);
							}
						} else{
							foreach ( $usermeta as $metakey => $metavalue ) {
								$metavalue = maybe_unserialize( $metavalue );
