...
if( !function_exists( 'doctreat_core_load_last' ) ) {
	function doctreat_core_load_last() {
		...
		$doctreat_plugin 		= plugin_basename(trim($wp_path_to_this_file));
		$active_plugins_list 	= get_option('active_plugins');
		$doctreat_plugin_key 	= array_search($doctreat_plugin, $active_plugins_list);
			array_splice($active_plugins_list, $doctreat_plugin_key, 1);
			array_push($active_plugins_list, $doctreat_plugin);
			update_option('active_plugins', $active_plugins_list);
	}
