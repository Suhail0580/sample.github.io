...
if( !function_exists( 'doctreat_process_registration' ) ){
	function doctreat_process_registration(){
		...
		$validations = array(
			...
			'am_mobile_number' 	=> esc_html__('Phone number field is required.', 'doctreat_core'),
		...
		$am_mobile_number	= !empty( $am_mobile_number ) ? $am_mobile_number : '';
        ...
        if ( is_wp_error( $user_identity ) ) {
			...
			if( !empty($user_type) && $user_type === 'seller'){
				...
				if( !is_wp_error( $post_id ) ) {
					...
					$profile_data['am_mobile_number']	= $am_mobile_number;
