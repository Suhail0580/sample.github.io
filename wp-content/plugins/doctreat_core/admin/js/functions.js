...
jQuery(document).ready(function($) {
	...
	jQuery(document).on('click', '.save-data-settings', function(event) {
		...
		var dataString 	   = 'security='+scripts_vars.ajax_nonce+'&'+serialize_data + '&action=doctreat_save_theme_settings';
	...
	jQuery(document).on('click', '.am-latest-mailchimp-list', function(event) {
		event.preventDefault();
		var dataString = 'security='+scripts_vars.ajax_nonce+'&action=doctreat_mailchimp_array';
	...
	jQuery(document).on('click', '.am-get-list', function(event) {
		event.preventDefault();
		var dataString = 'security='+scripts_vars.ajax_nonce+'&action=doctreat_cron_interval_array';
			...
			buttons: {
				yes: {
					...
					action: function () {
						...
						var _type		= _this.data('type'); 
						var dataString = 'security='+scripts_vars.ajax_nonce+'&type='+_type+'&id='+_id+'&user_id='+_user_id+'&action=doctreat_approve_profile';
						var jc	= this; 
						jc.showLoading();
						jQuery.ajax({
							type: "POST",
		 ...
		 jQuery.confirm({
			...
			'buttons': {
				'Yes': {
					'class': 'blue',
					'action': function () {
						var dataString = 'security='+scripts_vars.ajax_nonce+'&action=doctreat_import_users';
